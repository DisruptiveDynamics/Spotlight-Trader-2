      ...
      >
        {ticks.length === 0 ? (
          ...
          ticks.map((tick, i) => (
            <div
              key={`${tick.ts}-${i}`}
              className={`grid grid-cols-4 gap-2 px-3 py-1.5 border-b border-gray-800 hover:bg-gray-800/50 ${getTickColor(
                tick.side,
              )}`}
            >
              <div className="text-xs text-gray-500 font-mono">{formatTime(tick.ts)}</div>
              <div className="text-right font-bold">${tick.price.toFixed(2)}</div>
              <div className="text-right">{tick.size.toLocaleString()}</div>
