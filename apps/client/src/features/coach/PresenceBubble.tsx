...
export function PresenceBubble({ compact = false }: PresenceBubbleProps) {
  ...
  useEffect(() => {
    ...
    // Subscribe to coach state changes
    const unsubscribeCoachState = client.onCoachStateChange((state) => {
      setCoachState(state);
    });
    ...
    clickTimeoutRef.current = window.setTimeout(async () => {
      ...
      if (
        ...
        try {
          ...
          // Fetch voice proxy token
          const token = await fetchVoiceToken();
          tokenRef.current = token;
          
          // Connect to voice proxy (which has tools and real-time data)
  ...
  const handleKeyDown = async (e: KeyboardEvent) => {
    if (e.key === 't' || e.key === 'T') {
      ...
      if (connectionState === 'connected' && voiceClientRef.current) {
        try {
          const isMicActive = voiceClientRef.current.isMicActive();
          if (isMicActive) {
            voiceClientRef.current.disableMic();
            showStatusMessage('Muted 🔇', 1500);
          } else {
            await voiceClientRef.current.enableMic();
            showStatusMessage('Unmuted 🔊', 1500);
          }
        } catch (error) {
          console.error('Failed to toggle mute:', error);
