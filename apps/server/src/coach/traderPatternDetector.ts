...
export class TraderPatternDetector {
  ...
  async analyzeRecentBehavior(userId: string): Promise<TraderPattern[]> {
    ...
    const recentEvents = await db
      .select()
      .from(journalEvents)
      .where(and(eq(journalEvents.userId, userId), gte(journalEvents.timestamp, cutoffDate)))
    ...
    // Detect late entries
    const lateEntries = recentEvents.filter(
      (e) =>
    ...
    const chasing = recentEvents.filter(
      (e) => e.decision === "accept" && e.reasoning?.toLowerCase().includes("chase"),
    );
    ...
    const fomo = recentEvents.filter(
      (e) =>
    ...
    // Detect oversizing
    const oversizing = recentEvents.filter(
      (e) =>
    ...
    const recentLosses = recentEvents.filter(
      (e) => e.type === "exit" && e.realizedR !== null && e.realizedR < 0,
    );

    if (recentLosses.length >= 2) {
      // Check for quick successive trades after losses (within 30 minutes)
      const sortedByTime = [...recentLosses].sort(
        (a, b) => (b.timestamp?.getTime() || 0) - (a.timestamp?.getTime() || 0),
      );

      for (let i = 0; i < sortedByTime.length - 1; i++) {
        const timeDiff =
          (sortedByTime[i]?.timestamp?.getTime() || 0) -
        ...
        if (timeDiff < 30 * 60 * 1000) {
          patterns.push({
            ...
            severity: "high",
            recommendation:
              "Stop trading. Take a 30-minute break after losses. Revenge trading destroys accounts.",
          });
          break;
        }
      }
  ...
  async checkForPattern(
    ...
    const relevantPattern = patterns.find((p) => {
      if (p.symbol === symbol) {
        ...
        switch (p.type) {
          case "late_entry":
            return lowerReasoning.includes("late") || lowerReasoning.includes("chase");
          case "chasing":
            return lowerReasoning.includes("chase");
          case "fomo":
            return lowerReasoning.includes("fomo");
          case "oversizing":
            return lowerReasoning.includes("oversized");
          default:
            return false;
        }
      }
