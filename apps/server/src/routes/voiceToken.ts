...
export function setupVoiceTokenRoute(app: Express) {
  ...
  app.post("/api/voice/token", requireUser, async (req: AuthRequest, res) => {
    try {
      ...
      const toolsBridgeToken = jwt.sign({ userId, sessionId }, env.AUTH_JWT_SECRET, {
        expiresIn: "1h",
      });
  ...
  app.post("/api/voice/ephemeral-token", requireUser, async (req: AuthRequest, res) => {
    try {
      ...
      console.log(
        `[VOICE] Ephemeral token requested by user: ${userId} from IP: ${clientIp} (${rateLimit.remaining} remaining)`,
      );
