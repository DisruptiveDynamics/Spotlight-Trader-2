...
**Chart Overlay Management (Oct 2025)**: Production-grade indicator lifecycle with `upsertLineSeries` pattern - creates series once and updates data only, maintaining proper layer ordering above candlesticks. Implemented active overlay tracking with `activeIds` Set to automatically remove stale series when indicators are toggled off, preventing orphaned chart layers. Session-aware volume bar styling uses `bar_start` timestamps (not `bar_end`) for accurate RTH/extended hours detection across all code paths: live bars use `bar.bar_start`, historical data uses `msStart` field, bar reset uses `bar.bar_start`. Ensures consistent opacity rules (RTH=1.0, extended=0.3) with proper DST handling via `getVolumeColor()` utility.

**Enterprise Package Management (Oct 2025)**: Production-grade monorepo dependency organization with correct dev/runtime split. Root package contains tooling only (eslint, typescript, prettier), shared package declares date-fns/date-fns-tz as dependencies, server has compression/cookie runtime deps with proper type declarations, client has date-fns for history.ts with testing libs as devDependencies. Eliminates hoisting issues and missing module errors. Repository hygiene includes .gitattributes for line endings, comprehensive .gitignore with logs/diagnostics, and removal of duplicate config files.
